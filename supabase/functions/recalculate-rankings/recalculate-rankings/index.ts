...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: classes, error: classesError } = await supabaseClient.from('classes').select('id, name');
    ...
    for (const classItem of classes || []){
      try {
        ...
        const { data: members, error: membersError } = await supabaseClient.from('class_members').select('user_id').eq('class_id', classItem.id).eq('role', 'student');
        ...
        const { data: profiles, error: profilesError } = await supabaseClient.from('gamification_profiles').select(`
            user_id,
            xp_total,
            level,
            profiles (
          ...
          `).in('user_id', userIds).order('xp_total', {
          ascending: false
        });
        ...
        const { error: insertError } = await supabaseClient.from('class_rank_snapshots').insert(snapshot);
