...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: schools, error: schoolsError } = await supabaseClient.from('schools').select('id, name');
    ...
    for (const school of schools || []){
      try {
        ...
        // 2.1. Professores
        const { data: teachers } = await supabaseClient.from('school_teachers').select('user_id, status').eq('school_id', school.id);
        ...
        // 2.2. Turmas
        const { data: schoolClasses } = await supabaseClient.from('school_classes').select('class_id').eq('school_id', school.id);
        ...
        // 2.3. Alunos
        const { count: studentCount } = await supabaseClient.from('class_members').select('user_id', {
          count: 'exact',
          head: true
        }).in('class_id', classIds).eq('role', 'student');
        aggregation.metrics.total_students = studentCount || 0;
        // 2.4. Atividades das turmas
        const { data: activities } = await supabaseClient.from('activity_class_assignments').select('activity_id, activities(id, due_date)').in('class_id', classIds);
        const activityIds = activities?.map((a)=>a.activity_id) || [];
        if (activityIds.length > 0) {
          // 2.5. Submissões hoje
          const { count: submissionsToday } = await supabaseClient.from('submissions').select('id', {
            count: 'exact',
            head: true
          }).in('activity_id', activityIds).gte('submitted_at', today);
          aggregation.metrics.submissions_today = submissionsToday || 0;
          // 2.6. Submissões semana
          const { count: submissionsWeek } = await supabaseClient.from('submissions').select('id', {
            count: 'exact',
            head: true
          }).in('activity_id', activityIds).gte('submitted_at', weekAgo);
          aggregation.metrics.submissions_week = submissionsWeek || 0;
          // 2.7. Submissões mês
          const { count: submissionsMonth } = await supabaseClient.from('submissions').select('id', {
            count: 'exact',
            head: true
          }).in('activity_id', activityIds).gte('submitted_at', monthAgo);
          aggregation.metrics.submissions_month = submissionsMonth || 0;
          // 2.8. Taxa de pontualidade (semana)
          const { data: weekSubmissions } = await supabaseClient.from('submissions').select('submitted_at, activity_id').in('activity_id', activityIds).gte('submitted_at', weekAgo);
          if (weekSubmissions && weekSubmissions.length > 0) {
            const activityDueDates = new Map(activities?.map((a)=>[
                a.activity_id,
                a.activities?.due_date
              ]) || []);
            let onTimeWeek = 0;
          ...
          // 2.9. Média geral
          const { data: grades } = await supabaseClient.from('submissions').select('grade').in('activity_id', activityIds).not('grade', 'is', null);
        ...
        const studentIds = await supabaseClient.from('class_members').select('user_id').in('class_id', classIds).eq('role', 'student');
        const userIds = studentIds.data?.map((s)=>s.user_id) || [];
        if (userIds.length > 0) {
          // XP hoje
          const { data: xpToday } = await supabaseClient.from('xp_log').select('xp').in('user_id', userIds).gte('created_at', today);
          aggregation.metrics.xp_earned_today = xpToday?.reduce((sum, x)=>sum + x.xp, 0) || 0;
          // XP semana
          const { data: xpWeek } = await supabaseClient.from('xp_log').select('xp').in('user_id', userIds).gte('created_at', weekAgo);
          aggregation.metrics.xp_earned_week = xpWeek?.reduce((sum, x)=>sum + x.xp, 0) || 0;
          // Usuários ativos (hoje)
          const { data: activeToday } = await supabaseClient.from('gamification_profiles').select('user_id').in('user_id', userIds).gte('last_activity_at', today);
          aggregation.metrics.active_users_today = activeToday?.length || 0;
          // Usuários ativos (semana)
          const { data: activeWeek } = await supabaseClient.from('gamification_profiles').select('user_id').in('user_id', userIds).gte('last_activity_at', weekAgo);
