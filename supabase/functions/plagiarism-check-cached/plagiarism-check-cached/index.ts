...
serve(async (req)=>{
  ...
  try {
    const { submission_id, text, activity_id, user_id } = await req.json();
    ...
    const result = {
      ...
      is_plagiarized: (winstonData.similarity_score || 0) > 35,
    ...
    if (result.is_plagiarized) {
      ...
      const { data: activity } = await supabase.from('activities').select('plagiarism_threshold, class_id').eq('id', activity_id).single();
      const threshold = activity?.plagiarism_threshold || 35;
      if (result.similarity_score >= threshold) {
        // Get class_id from activity_class_assignments
        const { data: assignments } = await supabase.from('activity_class_assignments').select('class_id').eq('activity_id', activity_id).limit(1).single();
