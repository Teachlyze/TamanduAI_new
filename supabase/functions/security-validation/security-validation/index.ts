...
async function checkRateLimit(key, action) {
  ...
  const { data: attempts, error } = await admin.from("security_logs").select("id").eq("key", key).eq("action", action).gte("created_at", windowStart.toISOString());
...
async function logSecurityEvent(key, action, status, metadata = {}) {
  const admin = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
...
serve(async (req)=>{
  ...
  try {
    ...
    const { action, data, user_id, validation_rules = {} } = body;
    if (!action || !data) {
      return new Response(JSON.stringify({
