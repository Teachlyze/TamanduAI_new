...
function extractTextFromSubmission(data) {
  if (!data) return '';
  let text = '';
...
async function checkPlagiarismWithWinston(text, winstonKey) {
  try {
...
// Notificar professor
async function notifyTeacher(supabase, submission, severity, score) {
  if (severity === 'none' || severity === 'low') {
    // Não notificar para severidade baixa
    return;
  }
  // Buscar professor da turma
  const { data: classData } = await supabase.from('classes').select('created_by').eq('id', submission.class_id).single();
  if (!classData) return;
  // Criar notificação
...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: pendingSubmissions, error: fetchError } = await supabaseClient.from('submissions_pending_plagiarism').select('*').limit(10) // Processar 10 por vez
    ;
    if (fetchError) {
    ...
    for (const submission of pendingSubmissions){
      try {
        ...
        await supabaseClient.from('submissions').update({
          plagiarism_check_status: 'in_progress'
        }).eq('id', submission.submission_id);
        // Buscar dados completos da submissão
        const { data: fullSubmission, error: subError } = await supabaseClient.from('submissions').select('data, attachments').eq('id', submission.submission_id).single();
        ...
        const { error: insertError } = await supabaseClient.from('plagiarism_checks_v2').insert({
          submission_id: submission.submission_id,
          activity_id: submission.activity_id,
          class_id: submission.class_id,
        ...
        await supabaseClient.from('plagiarism_checks_v2').insert({
          submission_id: submission.submission_id,
          activity_id: submission.activity_id,
          class_id: submission.class_id,
          error: error.message,
