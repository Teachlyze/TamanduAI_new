...
serve(async (req)=>{
  ...
  try {
    ...
    const contextId = body.contextId; // meeting_id or event_id
    const bucket = body.bucket; // e.g., 'meeting-attachments' or 'event-attachments'
    const filePath = body.filePath; // path inside bucket
    const originalName = body.originalName || filePath.split("/").pop();
