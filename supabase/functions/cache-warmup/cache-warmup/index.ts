...
async function warmupUserProfile(admin, userId) {
  try {
...
async function warmupClassData(admin, classId) {
  try {
    const { data: classData } = await admin.from("classes").select(`
        *,
...
async function warmupActivityData(admin, activityId) {
  try {
    const { data: activity } = await admin.from("activities").select(`
        *,
...
async function warmupNotifications(admin, userId) {
  try {
    const { data: notifications } = await admin.from("notifications").select("*").eq("user_id", userId).order("created_at", {
      ascending: false
    }).limit(20);
...
async function warmupClassActivities(admin, classId) {
  try {
    const { data: activities } = await admin.from("activities").select("*").eq("class_id", classId).order("created_at", {
      ascending: false
    });
    return activities;
  } catch (error) {
...
async function warmupClassMembers(admin, classId) {
  try {
    const { data: members } = await admin.from("class_members").select("*, user:profiles(*)").eq("class_id", classId);
    return members;
  } catch (error) {
...
serve(async (req)=>{
  ...
  try {
    ...
    const { cache_keys = [], user_id, metadata = {} } = body;
    ...
    if (user_id) {
      const userWarmupTasks = [
        {
          name: "user_profile",
          task: ()=>warmupUserProfile(admin, user_id)
        },
        {
          name: "user_notifications",
          task: ()=>warmupNotifications(admin, user_id)
        },
        {
          name: "user_classes",
          task: async ()=>{
            const { data: classes } = await admin.from("class_members").select("class_id").eq("user_id", user_id);
