...
serve(async (req) => {
  ...
  try {
    ...
    if (cached) {
      ...
      return new Response(JSON.stringify({ user: cached }), { status: 200, headers: { 'Content-Type': 'application/json' } })
    }
    ...
    const { data: profile, error: profErr } = await admin
      .from('profiles')
      .select('id, full_name, avatar_url, role, email_confirmed, terms_accepted, privacy_accepted')
      .eq('id', userId)
      .single()
