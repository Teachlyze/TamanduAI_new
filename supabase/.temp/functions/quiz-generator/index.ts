...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return new Response(JSON.stringify({ error: "openai_error", detail: err }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (save) {
      try {
        ...
        const { data: quizRow, error: quizErr } = await supabase
          .from('quizzes')
          .insert({ title, topic, grade, difficulty, is_public: isPublic, created_by: userId })
          .select('id')
          .single();
        ...
        return new Response(
          JSON.stringify({ topic, grade, difficulty, quiz, saved: false, error: String(persistErr) }),
          { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 200 },
        );
      }
    }

    return new Response(JSON.stringify({ topic, grade, difficulty, quiz, saved: !!savedQuizId, quizId: savedQuizId }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
  } catch (e) {
