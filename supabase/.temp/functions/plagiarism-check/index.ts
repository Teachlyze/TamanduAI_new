...
function classifySeverity(percent: number): 'none'|'medio'|'grave'|'gravissimo' {
...
serve(async (req) => {
  ...
  try {
    ...
    const text: string | undefined = body.text; // prefer text
    const website: string | undefined = body.url || body.website; // optional
    const excluded_sources: string[] | undefined = body.exclude_urls || body.excluded_sources;
    const language: string | undefined = body.language || 'en';
    const country: string | undefined = body.country || 'us';

    if (!submissionId || (!text && !website)) {
      return new Response(JSON.stringify({ error: 'missing submissionId or content (text/website)' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    const token = Deno.env.get('WINSTON_AI_API_KEY');
    if (!token) {
      return new Response(JSON.stringify({ error: 'WINSTON_AI_API_KEY not configured' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    // Insert processing row
    const { data: checkRow, error: insErr } = await supabase
      .from('plagiarism_checks_v2')
      .insert({ submission_id: submissionId, activity_id: activityId ?? null, class_id: classId ?? null, status: 'processing', provider: 'winston' })
      .select('*')
      .single();
    ...
    if (!res.ok) {
      ...
      await supabase
        .from('plagiarism_checks_v2')
        .update({ status: 'failed', error: `Winston API ${res.status}: ${errText?.slice(0, 200)}` })
        .eq('id', checkRow.id);
      return new Response(JSON.stringify({ success: false, error: 'winston_api_error' }), { status: 502, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
      ...
      if (sub) {
        ...
        if (teacherId) {
          await supabase.from('notifications').insert({
            ...
            metadata: { classId: cls?.id, activityId: activity?.id, submissionId },
          });
        }
      }
    }

    return new Response(JSON.stringify({ success: true, checkId: checkRow.id, severity }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (e) {
