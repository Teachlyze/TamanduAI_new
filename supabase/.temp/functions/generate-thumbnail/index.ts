...
serve(async (req) => {
  ...
  try {
    ...
    const { bucket, filePath, width = 300, height = 300, quality = 80 } = await req.json() as ThumbnailRequest;
    ...
    if (downloadError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to download original file', details: downloadError.message }),
    ...
    if (!fileType.startsWith('image/')) {
      return new Response(
        JSON.stringify({ error: 'File is not an image', type: fileType }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
