...
function severityFromScore(score: number, t: { low: number; medium: number; high: number }) {
...
async function callWinstonAI(text: string) {
...
async function sendEmail(to: string, subject: string, html: string) {
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from: "alerts@tamandu.ai", to, subject, html }),
  });
...
serve(async (req) => {
  ...
  try {
    ...
    if (!submission_id || !activity_id || !text) {
      return new Response(JSON.stringify({ error: "Missing submission_id, activity_id, or text" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    let classOwner: string | null = null;
    if (effectiveClassId) {
    ...
    if (classOwner) {
      const { data: settings } = await admin
        ...
        .select("low_threshold, medium_threshold, high_threshold, notify_email, notify_in_app")
        .eq("owner_id", classOwner)
        .maybeSingle();
      if (settings) {
        thresholds = {
    ...
    if (!recheck) {
      const { data: existing } = await admin
        ...
        .maybeSingle();
      if (existing) {
        return new Response(JSON.stringify({ skipped: true, reason: "already_checked" }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        });
      }
      ...
      await admin.from("notifications").insert([
        {
          ...
          metadata: { submission_id, activity_id, score, severity },
          created_at: new Date().toISOString(),
        },
      ]);
    }
    ...
    if (notifyEmail && emailTo) {
      try {
        await sendEmail(
          emailTo,
          "Plagiarism alert",
          `<p>Submission ${submission_id} scored <b>${Math.round(score * 100)}%</b> (${severity}).</p>`
        );
      } catch (e) {
    ...
    return new Response(
      JSON.stringify({ ok: true, score, severity, check_id: checkRow?.id }),
