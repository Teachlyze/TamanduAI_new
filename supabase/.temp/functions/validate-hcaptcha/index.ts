...
serve(async (req) => {
  ...
  try {
    ...
    if (!token) {
      return new Response(JSON.stringify({ error: 'Token do hCaptcha não enviado.' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }
  ...
  if (!verifyData.success) {
    return new Response(JSON.stringify({ success: false, error: verifyData['error-codes'] || 'Verificação falhou' }), {
      status: 401,
      headers: { 'Content-Type': 'application/json' }
    });
  }
