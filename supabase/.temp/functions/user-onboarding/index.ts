...
serve(async (req) => {
  ...
  try {
    ...
    if (!full_name || !role || typeof terms_accepted !== 'boolean' || typeof privacy_accepted !== 'boolean') {
      return new Response(JSON.stringify({ error: 'Missing required fields: full_name, role, terms_accepted, privacy_accepted' }), { status: 400, headers: { 'Content-Type': 'application/json' } })
    }
    ...
    if (updErr) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to update onboarding' }), { status: 500, headers: { 'Content-Type': 'application/json' } })
    }
    ...
    return new Response(JSON.stringify({ user: userPayload }), { status: 200, headers: { 'Content-Type': 'application/json' } })
  } catch (e) {
