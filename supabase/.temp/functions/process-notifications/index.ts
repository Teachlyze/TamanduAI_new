...
async function processCalendarReminders(supabase: ReturnType<typeof createClient>) {
  const now = new Date();
  ...
  const { data: events, error: evErr } = await supabase
    ...
    .select('id, title, end_time, activity_id')
    .gt('end_time', now.toISOString())
    .lte('end_time', in48h);
  if (evErr) throw evErr;
  if (!events || events.length === 0) return { scheduled: 0 };

  // Preload activity metadata for improved timing
  const activityIds = Array.from(new Set(events.map((e: any) => e.activity_id).filter(Boolean)));
  let activityMap: Record<string, any> = {};
  if (activityIds.length > 0) {
    const { data: acts } = await supabase
      .from('class_activities')
      .select('id, estimated_minutes, complexity')
      .in('id', activityIds);
    activityMap = Object.fromEntries((acts || []).map((a: any) => [a.id, a]));
  }
  ...
  for (const ev of events) {
    ...
    const reminders = [
      { label: 'Iniciar preparo', when: new Date(end.getTime() - (workload + 120) * 60000) },
      { label: 'Lembrete 24h', when: new Date(end.getTime() - 24 * 60 * 60000) },
      { label: 'Lembrete 2h', when: new Date(end.getTime() - 2 * 60 * 60000) },
    ...
    const userIds: string[] = (attendees || []).map((a: any) => a.user_id);
      ...
      for (const r of reminders) {
        ...
        if (!exists) {
          await supabase.from('scheduled_notifications').insert({
            ...
            metadata: { end_time: ev.end_time, activity_id: ev.activity_id },
          });
          scheduled++;
        }
      }
...
async function processCalendarConflicts(supabase: ReturnType<typeof createClient>) {
  const now = new Date();
  ...
  const { data: events, error: evErr } = await supabase
    ...
    .lte('start_time', to);
  if (evErr) throw evErr;
  if (!events || events.length === 0) return { conflicts: 0 };
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      ...
      if (new Date(b.start_time) < new Date(a.end_time)) {
        ...
        const { data: exists } = await supabase
          ...
          .select('id')
          .eq('user_id', uid)
          .eq('reference_type', 'calendar_conflict')
...
async function processDueScheduledNotifications(supabase: ReturnType<typeof createClient>) {
  ...
  if (!due || due.length === 0) return { sent: 0, failed: 0, total: 0 };
  ...
  for (const n of due) {
    try {
      ...
      const { error: upErr } = await supabase
        .from("scheduled_notifications")
        .update({ executed_at: new Date().toISOString(), updated_at: new Date().toISOString() })
        .eq("id", n.id);
...
async function sendEmail(supabase: ReturnType<typeof createClient>, to: string, subject: string, html?: string, text?: string) {
  try {
...
async function processMissedDeadlines(supabase: ReturnType<typeof createClient>) {
  const now = new Date();
  ...
  if (!activities || activities.length === 0) return { checked: 0, notified: 0 };
  ...
  for (const act of activities) {
    const { data: enrollments, error: enrErr } = await supabase
      ...
      .select("student_id")
      .eq("class_id", act.class_id);
    if (enrErr) throw enrErr;

    const studentIds: string[] = (enrollments || []).map((e: any) => e.student_id);
    ...
    const emailById = Object.fromEntries((profiles || []).map((p: any) => [p.id, p.email]));
    ...
    for (const sid of studentIds) {
      ...
      if (!submission) {
        ...
        await supabase.from("notifications").insert({
          ...
          metadata: { classId: act.class_id, missedDeadline: true },
        });
        ...
        if (to) {
          await sendEmail(
            supabase,
            to,
            `Prazo vencido: ${act.title || "Atividade"}`,
            `<h2>Prazo vencido</h2><p>${act.title || "Atividade"} não foi entregue até ${act.due_date}</p>`
          );
        }
        notified++;
      }
  ...
  return { checked: activities.length, notified };
}
