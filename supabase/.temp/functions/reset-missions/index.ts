...
serve(async (req) => {
  ...
  try {
    ...
    const { data: expiredMissions, error: fetchError } = await supabaseClient
      .from('user_missions')
      .select(`
        id,
        user_id,
    ...
    // 2. Filtrar por tipo
    const missionsToReset = type === 'all' 
      ? expiredMissions
      : expiredMissions.filter((m: any) => m.missions_catalog?.type === type)
