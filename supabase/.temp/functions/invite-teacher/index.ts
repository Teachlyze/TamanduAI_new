...
serve(async (req: Request) => {
  ...
  try {
    ...
    const schoolId: string = (body?.schoolId || "").toString().trim();
    const schoolName: string | undefined = body?.schoolName;
    const inviterName: string | undefined = body?.inviterName;
    const message: string | undefined = body?.message;
    const inviteRedirectTo: string | undefined = body?.inviteRedirectTo; // opcional: URL onde o usuário deve ser redirecionado após aceitar

    if (!email || !schoolId) {
      return new Response(JSON.stringify({ error: "email and schoolId are required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (inviteError) {
      return new Response(JSON.stringify({ error: inviteError.message || String(inviteError) }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    return new Response(JSON.stringify({ ok: true, invited: true, emailSent, inviteData, emailError }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
