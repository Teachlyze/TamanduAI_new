  ...
  const LoadingComponent = useMemo(() => {
    ...
    switch (type) {
      ...
      case LOADING_TYPES.SKELETON: {
        return <SkeletonLoader variant={skeletonVariant} lines={skeletonLines} />;
      }

      default: {
        return <Loader2 className={`${sizeClass} animate-spin ${colorClass}`} />;
      }
    }
    ...
    <div className="flex justify-center items-center h-screen bg-gray-50 dark:bg-gray-900">
      <div className="flex flex-col items-center space-y-6">
        ...
        <div className="relative">
          ...
          {type === LOADING_TYPES.SPINNER && (
            <div className={`absolute inset-0 ${size === 'xl' ? 'w-16 h-16' : size === 'lg' ? 'w-12 h-12' : 'w-8 h-8'} rounded-full bg-gradient-to-r from-blue-500 to-purple-500 opacity-20 animate-pulse`}></div>
          )}
        </div>
        ...
        <div className="text-center">
          ...
          <div className="flex items-center justify-center space-x-1 mt-2">
            <div className={`w-1 h-1 bg-current ${colorClasses[color] || 'text-blue-500'} rounded-full animate-bounce`}></div>
            <div className={`w-1 h-1 bg-current ${colorClasses[color] || 'text-blue-500'} rounded-full animate-bounce`} style={{ animationDelay: '0.1s' }}></div>
            <div className={`w-1 h-1 bg-current ${colorClasses[color] || 'text-blue-500'} rounded-full animate-bounce`} style={{ animationDelay: '0.2s' }}></div>
          </div>
        </div>
