...
const Sidebar = ({ open, setOpen }) => {
    ...
    (async () => {
      ...
      try {
        ...
        if (error) {
          ...
          console.warn(
            "[Sidebar] Erro ao buscar role, defaulting para student:",
            error
          );
          setRole("student");
          return;
  ...
  const teacherNav = [
    ...
    },
    {
      name: "Rascunhos",
      icon: BookOpen,
      path: "/dashboard/activities/drafts",
      tourId: "nav-activities-drafts",
    },
    {
      name: "Relatórios",
      icon: BarChart3,
  ...
  const getBottomItems = () => {
    const settingsPath =
      role === "school" ? "/school/settings" : "/dashboard/settings";
    return [
      ...
      >
        <div className="p-4 flex items-center justify-between border-b border-border/50 bg-gradient-to-r from-primary/5 to-purple-500/5 text-white hover:opacity-90">
          <div className="flex items-center">
            ...
            <span
              className={`text-xl font-bold bg-gradient-to-r from-primary via-purple-600 to-indigo-600 bg-clip-text text-transparent ${open ? "" : "lg:hidden"}`}
            >
              TamanduAI
            </span>
          </div>
            ...
            {navItems.map((item) => (
              <li key={item.name}>
                {item.disabled ? (
                  ...
                  >
                    <item.icon
                      className={`w-5 h-5 text-muted-foreground/30 ${open ? "mr-3" : "lg:mx-auto"}`}
                    />
              ...
              ${
                location.pathname === item.path
                  ...
                  >
                    <item.icon
                      className={`w-5 h-5 ${open ? "mr-3" : "lg:mx-auto"}`}
                    />
                    <span className={`${open ? "" : "lg:hidden"}`}>
                      {item.name}
                    </span>
