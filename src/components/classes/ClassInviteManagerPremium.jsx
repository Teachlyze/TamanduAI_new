  ...
  const getStatusBadge = (invite) => {
    if (invite.status !== "active") {
      return (
        <Badge
          variant="outline"
          className="bg-white dark:bg-slate-900 text-foreground border-border text-xs bg-red-100 text-red-700 dark:bg-red-900/20"
        >
          Cancelado
        </Badge>
      );
    ...
    if (new Date(invite.expires_at) < new Date()) {
      return (
        <Badge
          variant="outline"
          className="bg-white dark:bg-slate-900 text-foreground border-border text-xs bg-orange-100 text-orange-700 dark:bg-orange-900/20"
        >
          Expirado
        </Badge>
      );
    ...
    if (invite.current_uses >= invite.max_uses) {
      return (
        <Badge
          variant="outline"
          className="bg-white dark:bg-slate-900 text-foreground border-border text-xs bg-gray-100 text-gray-700 dark:bg-gray-900/20"
        >
          Esgotado
        </Badge>
      );
    ...
    return (
      <Badge
        variant="outline"
        className="bg-white dark:bg-slate-900 text-foreground border-border text-xs bg-green-100 text-green-700 dark:bg-green-900/20"
      >
        Ativo
      </Badge>
    );
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 text-xs text-muted-foreground">
                            ...
                            {invite.role && (
                              <Badge
                                variant="outline"
                                className="bg-white dark:bg-slate-900 text-foreground border-border text-xs"
                              >
                                {invite.role === "student"
                                  ? "👨‍🎓 Aluno"
                                  : "👨‍🏫 Professor"}
                    ...
                    <div className="mb-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-sm flex-1 truncate">
                          {window.location.origin}/join-class/
                          {invite.invitation_code}
                        </span>
                      </div>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {Math.round(
                                (invite.current_uses / invite.max_uses) * 100
                              )}
                              %
                            </span>
                          </div>
