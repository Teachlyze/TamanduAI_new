...
const SubmissionManager = ({
    ...
    try {
      ...
      (submissions || []).forEach((s) => {
        ...
        if (Array.isArray(raw)) {
          map[s.id] = raw.map((item, idx) => ({
            id: `${s.id}-${idx}`,
            question_index: idx + 1,
            value: item?.answer_text ?? String(item ?? ""),
            is_correct: null,
            points_earned: null,
          }));
        } else if (raw && typeof raw === "object") {
          const entries = Object.entries(raw);
          map[s.id] = entries.map(([key, val], idx) => ({
            id: `${s.id}-${key}`,
            question_index: idx + 1,
            value: val?.answer_text ?? String(val ?? ""),
            is_correct: null,
            points_earned: null,
          }));
        } else {
  ...
  const filteredSubmissions = submissions.filter((s) =>
    selectedGroupId === "all" ? true : s.group_id === selectedGroupId
  );
  ...
  const returnForRevision = async (submission) => {
    try {
      const message = prompt(
        ...
        "Por favor, revise sua submissão conforme os comentários."
      );
      ...
      if (onSubmissionsChange) {
        onSubmissionsChange(
          submissions.map((s) =>
            s.id === submission.id
              ? { ...s, status: "returned", feedback: message }
              : s
          )
        );
      }
      toast({
        title: "Devolvida para revisão",
  ...
  const saveGrading = async (submission) => {
    try {
      ...
      if (onSubmissionsChange) {
        onSubmissionsChange(
          submissions.map((s) => (s.id === subId ? { ...s, ...update } : s))
        );
      }
      toast({
        title: "Avaliação salva",
        description: "Nota e feedback atualizados com sucesso.",
      });
    } catch (e) {
  ...
  const checkPlagiarism = useCallback(
    async (submission) => {
      ...
      try {
          ...
          const updatedSubmissions = submissions.map((s) =>
            s.id === submission.id
              ? {
                  ...
                  plagiarism_score: data.similarity_score || 0,
                }
              : s
          );
        ...
        setError(
          "Não foi possível verificar plágio. Tente novamente mais tarde."
        );
        throw err;
      } finally {
  ...
  const getPlagiarismStatus = (submission) => {
    ...
    const score =
      submission.plagiarism_score || (result ? result.similarity_score : null);
  ...
  const hasPendingPlagiarismChecks = filteredSubmissions.some(
    (s) => !s.plagiarism_checked && isPlagiarismCheckEnabled
  );
                  ...
                  {isTeacher && (
                    <div className="mt-4 pl-14 space-y-3">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="border rounded-md divide-y">
                            {(answersMap[submission.id] || []).map((ans) => (
                              <div
                                key={ans.id}
                                className="p-2 grid grid-cols-1 lg:grid-cols-4 gap-2 items-center"
                              >
                                <div className="text-xs text-gray-600">
                                  Q{ans.question_index}
                                </div>
                                <div className="text-xs break-words lg:col-span-1">
                                  {String(ans.value)}
                                </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {(() => {
                          const { totalEarned, totalPossible } =
                            computeSubmissionScore(submission.id);
                          const grade =
                            submission.grade ??
                            (totalPossible > 0
