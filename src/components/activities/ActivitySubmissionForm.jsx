    ...
    try {
      ...
      for (const file of newFiles) {
        try {
          ...
          const { data: uploadedFile, error } = await uploadSubmission(
            file,
            tempId
          );
      ...
      setFormError(
        "Ocorreu um erro ao processar os arquivos. Tente novamente."
      );
      return [];
    } finally {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      ...
      if (!data.answer && (!data.files || data.files.length === 0)) {
        throw new Error(
          "Por favor, insira uma resposta ou anexe pelo menos um arquivo."
        );
      }

      if (isPlagiarismCheckEnabled && !data.agreeTerms) {
        throw new Error(
          "Você deve concordar com os termos de verificação de plágio."
        );
      }
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            ...
            <div className="mt-2 text-sm text-green-700">
              <p>
                Sua resposta foi registrada e está disponível para o professor.
              </p>
            </div>
          </div>
        </div>
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      ...
      {schema?.properties && (
        <div className="space-y-4">
          {Object.entries(schema.properties).map(([key, prop], idx) => {
            ...
            const setValueFor = (v) =>
              setAnswersByIndex((prev) => ({ ...prev, [qIndex]: v }));
              ...
              <div key={key} className="space-y-2">
                ...
                {qType === "textarea" && (
                  <textarea
                    ...
                    rows={4}
                    value={value || ""}
                    onChange={(e) => setValueFor(e.target.value)}
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mb-2">
          Você pode anexar até {maxFiles} arquivos (máx.{" "}
          {maxSize / (1024 * 1024)}MB por arquivo)
        </p>
        ...
        <div className="flex items-start">
          ...
          <div className="ml-3 text-sm">
            ...
            <p className="text-gray-500">
              Concordo que minha submissão será verificada quanto a plágio.
              Qualquer violação dos termos acadêmicos pode resultar em
              penalidades.
            </p>
            {errors.agreeTerms && (
