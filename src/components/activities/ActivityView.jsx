...
const ActivityView = ({ activityId, isTeacher = false, userId }) => {
  ...
  useEffect(() => {
    const loadActivity = async () => {
      try {
        ...
        setAssignedClasses(
          (assigns || []).map((a) => ({
            id: a.classes?.id || a.class_id,
            name: a.classes?.name || "Turma",
            color: a.classes?.color,
          }))
        );
        ...
        setError(
          "Não foi possível carregar a atividade. Tente novamente mais tarde."
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const loadSubmissions = async (activityId) => {
    try {
      ...
      setError(
        "Não foi possível carregar as submissões. Tente novamente mais tarde."
      );
    }
  };
  ...
  const handleSubmitActivity = async (submissionData) => {
    try {
      ...
      const payload = {
        data: submissionData?.answersByIndex
          ? submissionData.answersByIndex
          : (submissionData?.answer ?? null),
      ...
      if (activity?.settings?.plagiarism_check_enabled) {
        ...
        try {
          const { error: checkError } = await supabase.functions.invoke(
            "plagiarism-check",
            {
              body: JSON.stringify({
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500">
          A atividade que você está tentando acessar não existe ou foi removida.
        </p>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              {activity.total_points !== null &&
                activity.total_points !== undefined && (
                ...
                {userSubmission ? (
                  <div className="space-y-4">
                      ...
                      <div className="flex">
                        ...
                        <div className="ml-3">
                          <p className="text-sm text-green-700">
                            Você já enviou sua resposta em{" "}
                            {new Date(
                    ...
                    <div className="bg-white shadow overflow-hidden sm:rounded-lg">
                      ...
                      <div className="border-t border-gray-200 px-4 py-5 sm:px-6">
                        {userSubmission.data &&
                        typeof userSubmission.data === "string" ? (
                          <div
                            className="prose max-w-none"
                            dangerouslySetInnerHTML={{
                              __html: userSubmission.data,
                            }}
                          />
                        ) : userSubmission.data &&
                          Array.isArray(userSubmission.data) ? (
                          <ul className="list-disc pl-6">
                            {userSubmission.data.map((item, idx) => (
                              ...
                              >
                                {String(item?.answer_text ?? item)}
                              </li>
                            ))}
                          </ul>
                        ) : userSubmission.data &&
                          typeof userSubmission.data === "object" ? (
                          <ul className="list-disc pl-6">
                            {Object.entries(userSubmission.data).map(
                              ([k, v]) => (
                                <li
                                  key={k}
                                  className="text-sm text-gray-700 break-words"
                          ...
                          <p className="text-gray-500 italic">
                            Nenhuma resposta em texto fornecida.
                          </p>
                        )}
                          ...
                          userSubmission.attachments.length > 0 && (
                            <div className="mt-6">
                              ...
                              <ul className="border border-gray-200 rounded-md divide-y divide-gray-200">
                                {userSubmission.attachments.map(
                                  (file, index) => (
                                    <li
                                      key={index}
                  ...
                  <ActivitySubmissionForm
                    ...
                    isPlagiarismCheckEnabled={
                      activity.settings?.plagiarism_check_enabled || false
                    }
                  />
                )}
              </div>
