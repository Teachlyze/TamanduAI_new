...
const TourProvider = ({ children }) => {
  ...
  const startTour = useCallback(async (r) => {
    ...
    try {
      ...
      if (available.length < 2) {
        console.warn('[Tour] Not enough targets found for role', effectiveRole, available);
  ...
  const handleJoyrideCallback = useCallback(async (data) => {
    ...
    if (type === EVENTS.TOUR_END || (type === EVENTS.TARGET_NOT_FOUND && action === ACTIONS.CLOSE)) {
      setRun(false);
    }
