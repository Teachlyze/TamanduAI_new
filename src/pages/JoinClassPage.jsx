...
import { Loader2, Check, X, AlertCircle, UserPlus, GraduationCap, Users } from 'lucide-react';
...
export default function JoinClassPage() {
  ...
  useEffect(() => {
    const loadInviteDetails = async () => {
      ...
      try {
        setStatus('loading');
        const details = await ClassInviteService.getInviteDetails(invitationCode);
  ...
  const handleAcceptInvite = async () => {
    ...
    try {
      const result = await ClassInviteService.acceptInvite(invitationCode, user.id);
      ...
      if (result.success) {
        ...
        toast({
          title: result.alreadyMember ? 'Você já está na turma!' : 'Inscrição realizada!',
  ...
  return (
          <Card className="max-w-2xl mx-auto">
            <CardHeader>
              ...
              <CardTitle className="text-center">Você foi convidado para uma turma!</CardTitle>
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    <div className="space-y-2">
                      <p className="font-medium">Turma: {inviteDetails.class?.name}</p>
                      {inviteDetails.class?.description && (
                        <p className="text-sm text-muted-foreground">{inviteDetails.class.description}</p>
                      )}
                      <p className="text-sm">
                        Professor: {inviteDetails.class?.teacher?.full_name || 'Professor'}
                      </p>
                    </div>
                  </AlertDescription>
                </Alert>
          ...
          <Card className="max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="border rounded-lg p-6 space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 space-y-1">
                    <h3 className="text-xl font-semibold">{inviteDetails?.class?.name}</h3>
                    {inviteDetails?.class?.description && (
                      <p className="text-muted-foreground">{inviteDetails.class.description}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 pt-4 border-t">
                  <Avatar>
                    <AvatarImage src={inviteDetails?.class?.teacher?.avatar_url} />
                    <AvatarFallback>
                      {inviteDetails?.class?.teacher?.full_name?.charAt(0) || 'P'}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                ...
                <div className="flex items-center gap-2 pt-2">
                  ...
                  <span className="text-sm">
                    Você entrará como: <strong>{inviteDetails?.role === 'student' ? 'Aluno' : 'Professor'}</strong>
                  </span>
                </div>
              </div>
              ...
              <Alert>
                ...
                <AlertDescription>
                  Ao confirmar, você será adicionado à turma e terá acesso a todos os materiais e atividades.
                </AlertDescription>
              </Alert>
            </CardContent>
