import{s as i}from"./main-7VZoQpWp.js";const h=async(t,e,s,r="read")=>{try{const{data:{session:a}}=await i.auth.getSession();if(!a?.user)throw new Error("Usuário não autenticado");switch(t){case"class":{const{data:o,error:n}=await i.from("classes").select("created_by, id").eq("id",e).single();if(n)throw n;if(r==="write"){if(o.created_by!==s)throw new Error("Apenas o professor pode modificar esta turma")}else{if(o.created_by===s)return!0;const{data:c}=await i.from("class_members").select("id").eq("class_id",e).eq("user_id",s).eq("role","student").single();if(!c)throw new Error("Você não tem acesso a esta turma")}break}case"activity":{const{data:o,error:n}=await i.from("activities").select(`
            id,
            created_by
